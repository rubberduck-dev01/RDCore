...
public sealed record class VBByteType() : VBNumericType<byte>(VBTypeNames.VBByte), IIntegralNumericType
{
    ...
    private static readonly Lazy<VBByteValue> _defaultValue = new(() => (VBByteValue)new VBByteValue().WithValue(ManagedInteropValue<byte>.ByteZeroValue.Value), LazyThreadSafetyMode.PublicationOnly);
    public override VBByteValue DefaultValue => _defaultValue.Value;

    private static readonly Lazy<VBByteValue> _minValue = new(() => (VBByteValue)new VBByteValue().WithValue(ManagedInteropValue<byte>.ByteMinValue.Value), LazyThreadSafetyMode.PublicationOnly);
    /// <summary>
    /// Gets the minimum representable value for this data type.
    /// </summary>
    public static VBByteValue MinValue => _minValue.Value;
    public override double ManagedMinValue => (double)((ManagedInteropValue<byte>)_minValue.Value.ManagedValue.InteropValue!).StoredValue;

    private static readonly Lazy<VBByteValue> _maxValue = new(() => (VBByteValue)new VBByteValue().WithValue(ManagedInteropValue<byte>.ByteMaxValue.Value), LazyThreadSafetyMode.PublicationOnly);
    /// <summary>
    ...
    private static readonly Lazy<VBByteValue> _zero = new(() => (VBByteValue)new VBByteValue().WithValue(ManagedInteropValue<byte>.ByteZeroValue.Value), LazyThreadSafetyMode.PublicationOnly);
    /// <summary>
