...
public sealed record class VBDateType() : VBIntrinsicType<DateTime>(VBTypeNames.VBDate)
{
    ...
    private static readonly Lazy<VBDateValue> _minValue = new(() 
        => new VBDateValue { ManagedValue = new(new ManagedInteropValue<double>(new DateTime(100, 01, 01).ToOADate())) }, LazyThreadSafetyMode.PublicationOnly);
    ...
    private static readonly Lazy<VBDateValue> _maxValue = new(() 
        => new VBDateValue { ManagedValue = new(new ManagedInteropValue<double>(MaxSerial)) }, LazyThreadSafetyMode.PublicationOnly);
    ...
    private static readonly Lazy<VBDateValue> _zero = new(() 
        => new VBDateValue { ManagedValue = new(new ManagedInteropValue<double>(new DateTime(1899, 12, 30).ToOADate())) }, LazyThreadSafetyMode.PublicationOnly);
