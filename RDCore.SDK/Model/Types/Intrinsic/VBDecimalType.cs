...
public sealed record class VBDecimalType() : VBNumericType<decimal>(VBTypeNames.VBDecimal), IFixedPointNumericType
{
    ...
    public override double ManagedMinValue => Convert.ToDouble(_minValue.Value.ManagedValue.Decimal);
    ...
    public override double ManagedMaxValue => Convert.ToDouble(_maxValue.Value.ManagedValue.Decimal);

    private static readonly Lazy<VBDecimalValue> _zero = new(() => (VBDecimalValue)new VBDecimalValue(GlobalSymbols.ExtensionSymbols.VBDecimalZeroValue).WithValue(Values.ManagedValue.DecimalZeroValue), LazyThreadSafetyMode.PublicationOnly);
    /// <summary>
