...
> ⚠️ Because RD-VBA **identifier names are semantically case-insensitive**, folder names "`project1`" and "`Project1`" are **considered as identical** _regardless_ of whether or not the underlying file system supports it. 
...
> ℹ️ The value of the `Name` property of a `RDCoreModule` must be unique across the entire _workspace_ and is always supplied by a `VB_Name` _attribute_. In case of a mismatch with the value of a `VB_Name` attribute, _the attribute value always takes precedence_.
